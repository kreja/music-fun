...
~(function(doc) {
	...
	FallingLeaves.prototype = {
		...
		imgurl: function() {
			var i = 0,
				got = -1,
				len = doc.getElementsByTagName('script').length;
			while (i <= len && got == -1) {
				var js_url = doc.getElementsByTagName('script')[i].src,
					got = js_url.indexOf('leaves.js');
				i++
			}
			return js_url.replace('leaves.js', '../img/')
		},
		...
		createLeaf: function() {
			...
			image.src = self.imgurl() + self.randomInteger(1, self.num) + '.png';
